...
| AUTO_UPDATE          | Exports a .spi bitstream image for use with FPGA In-Application Programming (IAP)/auto-update flows. This option only exports the .spi file. This argument can be used with the mpfs-icicle-kit board (MPFS250T die), but not the mpfs-icicle-kit-es board (MPFS250T_ES die).                                                                                                                           |
| GOLDEN_IMAGE  | Creates a SPI Flash golden image (which is used as a back-up if updating to the runtime supplied image fails) and exports it to a SPI Flash only job file. This argument can be used with the mpfs-icicle-kit board (MPFS250T die), but not the mpfs-icicle-kit-es board (MPFS250T_ES die).                                                                                                                           |
...
| SILICON_SIGNATURE:SIGNATURE      | Used to specify the silicon signature to be included in the programming bitstream, set to '1c1c1e' by default for production silicon Icicle kit and '1c1c1ee5' by default for the Engineering sample Icicle Kit. Setting this argument overrides these default values.

| TOP_LEVEL_NAME:MY_DESIGN_NAME    | Specifies the name of the top level module. This will be the name of the FPGA design reported through System services and displayed at boot time by the Hart Software Services. This will allow you to identify which design is programmed on the board.
